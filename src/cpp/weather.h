...
public:
	...
	Weather(std::string filename, std::string start_date,std::string end_date){
		...
		std::vector<scalar> days=std::vector<scalar>(temperatures.size());//TODO:use utils.getdays?
		std::iota(days.begin(),days.end(),0);

        this->p = [precipitations](scalar t) { return precipitations[int(t)]* (sin(2.*M_PI*t + 3.*M_PI/2.) +1.); };//TODO:use function?
