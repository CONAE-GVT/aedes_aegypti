...
class Configuration:
  ...
  def validate(self):
    ...
    assert len(vBS_d) == len(vBS_h) == len(vBS_s) == len(vBS_W0) == len(vBS_mf) == len(vBS_b) == len(vBS_ef) == len(alpha0),'vBS_d, vBS_h, vBS_s, vBS_W0, vBS_mf and alpha0 must have the same dimension!'
